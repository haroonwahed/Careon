...
- Validates active provider/vestiging state.
- Enforces active regional coverage (`ProviderRegioDekking`) for case region.
- Enforces active contractability (`ContractRelatie`).
- Applies capacity viability checks (direct or waitlistable).
- Stores exclusion reasons (`uitgesloten`, `uitsluitreden`).
...
## Signal Hooks
Matching dashboard now emits/updates operational signals for:
