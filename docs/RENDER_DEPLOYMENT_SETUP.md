...
2. Build the frontend (Vite + Vue)
3. Collect Django static files
4. If `DATABASE_URL` is set: Run Django migrations
5. If `DATABASE_URL` is NOT set: Skip migrations (uses SQLite fallback for build)
...
**Cause:** `DATABASE_URL` is either not set or contains an invalid hostname (like `"your_db_host"`)
...
- Manually run migrations:
  ```bash
  render bash  # Connect to Render container
...
- ✅ Build succeeds even if database isn't ready yet
- ✅ Multiple replicas can coexist (only first startup runs migrations)
