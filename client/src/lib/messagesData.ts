...
const generateMessages = (): Message[] => {
  const accounts = [
    { id: "account1", name: "Boutique Alice", initial: "A", color: "var(--chart-1)" },
    { id: "account2", name: "Frip Tim", initial: "T", color: "var(--chart-2)" },
    { id: "account3", name: "Margo Vintage", initial: "M", color: "var(--chart-3)" },
  ];
  ...
  for (let dayOffset = 0; dayOffset < 7; dayOffset++) {
    ...
    for (let i = 0; i < messagesPerDay; i++) {
      ...
      messages.push({
        ...
        avatarColor: account.color || avatarColors[Math.floor(Math.random() * avatarColors.length)],
      });
    }
  }
