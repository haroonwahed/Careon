import { ArrowLeft, ArrowRight, AlertTriangle, Building2, CalendarClock, Clock3, MapPin, ShieldAlert, Sparkles } from "lucide-react";
...
function queueLabel(page: "casussen" | "beoordelingen" | "matching" | "plaatsingen" | "intake") {
  switch (page) {
    case "beoordelingen":
      return "Open beoordelingsqueue";
    case "matching":
...
function previousStepTarget(phase: string): "casussen" | "beoordelingen" | "matching" | "plaatsingen" | "intake" {
  switch (phase) {
    case "matching":
        ...
        <div className="rounded-2xl border bg-card p-6">
          <div className="flex items-start justify-between gap-6">
            <div className="max-w-2xl">
              ...
              <h2 className="text-2xl font-semibold text-foreground">{actionLabel}</h2>
              <p className="mt-2 text-sm text-muted-foreground">{workflowCase.workflowState.nextActionDetail}</p>
            </div>
            <div className="shrink-0 space-y-2">
              <Button onClick={handlePrimaryAction} className="w-full gap-2">
                {queueActionLabel}
                <ArrowRight size={16} />
              </Button>
              {previousTarget !== "casussen" && (
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]">
          <div className="space-y-6">
            <section className="rounded-2xl border bg-card p-5">
              <h3 className="mb-4 text-lg font-semibold text-foreground">Casus summary</h3>
            ...
            <section className="rounded-2xl border bg-card p-5">
              <h3 className="mb-4 text-lg font-semibold text-foreground">Beoordeling</h3>
              <div className="space-y-3 text-sm">
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">Status</span>
                  <span className="font-medium text-foreground">{workflowCase.phase === "intake" ? "Nog niet gestart" : workflowCase.phase === "beoordeling" ? "Lopend" : "Afgerond"}</span>
                </div>
                <div className="flex items-center justify-between">
            ...
            <section className="rounded-2xl border bg-card p-5">
              <h3 className="mb-4 text-lg font-semibold text-foreground">Matching</h3>
              <div className="space-y-3 text-sm">
                <div className="flex items-center justify-between">
            ...
            <section className="rounded-2xl border bg-card p-5">
              <h3 className="mb-4 text-lg font-semibold text-foreground">Plaatsing</h3>
              <div className="space-y-3 text-sm">
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">Status</span>
          ...
          <div className="space-y-6">
            ...
            <section className="rounded-2xl border bg-card p-5">
              <h3 className="mb-4 text-lg font-semibold text-foreground">System signals</h3>
              <div className="space-y-3">
                {workflowCase.workflowState.signals.length === 0 && <p className="text-sm text-muted-foreground">Geen actieve signalen.</p>}
