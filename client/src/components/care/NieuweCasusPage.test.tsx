...
describe("NieuweCasusPage", () => {
  ...
  it("shows the workflow-privacy guidance and step gate copy", async () => {
    ...
    const guidance = document.getElementById("nieuw-casus-page-guidance");
    expect(guidance).toBeInTheDocument();
    expect(within(guidance!).getByText("We koppelen deze casus aan een bronregistratie en bepalen de basiscontext.")).toBeInTheDocument();
    expect(within(guidance!).getByText("Vul alleen de minimale gegevens in om te starten. Aanvullende informatie volgt in de volgende stappen.")).toBeInTheDocument();
    expect(guidance).toHaveTextContent(/Persoonsgegevens blijven afgeschermd tot formele intake of koppeling/i);
    expect(guidance).toHaveTextContent(/Meer over privacy en zichtbaarheid/i);
    expect(guidance).toHaveTextContent(/Velden met \* zijn verplicht/i);
    expect(screen.queryByTestId("nieuwe-casus-privacy-ribbon")).not.toBeInTheDocument();
  ...
  it("supports keyboard navigation for choice groups and disclosure controls", async () => {
    ...
    expect(screen.getByText("We koppelen deze casus aan een bronregistratie en bepalen de basiscontext.")).toBeVisible();
