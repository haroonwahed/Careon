...
- `/care/api/cases/<int:case_id>/arrangement-alignment/`
...
| Regiekamer | Prioritize urgent cases and bottlenecks | Done | Regiekamer Lite is read-only, powered by `evaluate_case()`, and consumes the decision overview endpoint | `client/src/components/care/RegiekamerControlCenter.tsx`, `client/src/lib/regiekamerDecisionOverview.ts`, `contracts/api/views.py`, `contracts/decision_engine.py` | Keep it operational triage, not BI reporting | High |
...
| Decision engine | Evaluate a casus and surface blockers, risks, alerts, and next best action | Done | Backend `evaluate_case()` powers the case detail panel and the Regiekamer decision overview | `contracts/decision_engine.py`, `contracts/api/views.py`, `client/src/components/care/CaseWorkflowDetailPage.tsx`, `client/src/lib/decisionEvaluation.ts`, `client/src/lib/regiekamerDecisionOverview.ts` | Keep frontend consumption read-only | High |
